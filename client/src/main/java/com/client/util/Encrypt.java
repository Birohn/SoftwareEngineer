...
public class Encrypt {
    ...
    public String encode(String message, int key) {
        ...
        for (int i = 0; i < message.length(); i++) {
            ...
            if (chemSymbols.get((int) currentChar) != null)
    ...
    public String decode(String message, int key) {
        ...
        while (i < message.length()) {
            ...
            if (isLetter(currentPair.charAt(0)) && isLetter(currentPair.charAt(1))) {
                int decoded = (int) getKeyFromValue(chemU, currentPair);
                decoded2 = (char) decoded;
                output += decoded2;
            } else if (!isLetter(currentPair.charAt(0))) {
                output += currentPair.charAt(0);
                i++;
                continue;
            } else {
                output += decoded2;
